...
- **知識スレッド** — LLM 駆動の構造化知識進化システム。ノード、エッジ、ギャップ、メインラインステップ、次の方向性、スレッド間関係、エビデンス参照を含む。スレッド単位のイテレーション、関連度フィルタリング、ゴミ箱/復元に対応
- **クリエイティブ・インスピレーション・スイート** — アイデアファクトリー（SCAMPER、TRIZ、デザイン思考、構造的空隙、類推転移、反実仮想、エビデンス駆動など 9 手法）、テーマラボ、ドリームラボ。多段階の創造的進化とフィードバックシステム
- **WeChat FileHelper 統合** — QR コードログイン、メッセージ履歴同期、添付ファイルダウンロード、カード/リンクプレビュー解析、アプリ内チャット
- **論文リサーチプラットフォーム** — 学術論文 PDF のインポートと自動分析、LLM クエリ書き換えによる文献検索、arXiv デイリーモニタリング、論文候補の収集とバッチインポート
...
元の設計にはありません。Wiki 知識を進化する構造化された**知識スレッド**として LLM が組織化するシステムです。

**スレッドモデル:**
- 各スレッドは**名前**、**サマリー**（エビデンス参照付き）、**コア質問**（メカニズム/トレードオフ/進化/矛盾/応用/ギャップの 6 タイプ）、**ルートトピック**、**キーコンセプト**、**ソースページ**を持つ
- **スコア**で成熟度、カバレッジ、一貫性、新規性、アクティビティを追跡 — 各イテレーションで LLM が更新
- **ステータスライフサイクル**: forming → active → mature → stale → archived
- **生成モード**: `llm`（完全分析）、`local_candidate`（ヒューリスティック事前生成）、`repair`（修復）

**スレッドごとの知識グラフ:**
- **ノード** 8 タイプ: topic, concept, question, method, claim, case, gap, idea — 各ノードにソースページ参照、信頼度、重要度スコア
- **エッジ** 9 タイプの関係: contains, depends_on, supports, contradicts, evolves_to, inspires, derived_from, should_explore, related_to — 各エッジに理由と信頼度
- **ギャップ** 構造化メタデータ: ギャップタイプ、重要性、不足エビデンス、ソースノード/ページ参照、優先度、ステータス（open → investigating → resolved → dismissed → watching）
- **メインラインステップ** — 背景からコアコンセプト、キー質問、手法、エビデンス、事例、矛盾、ギャップ、次の方向性までの順序付きストーリーライン
- **次の方向性** — アクションタイプ（read_more, ask_user, web_research, connect_nodes, validate_claim, compare_threads, generate_idea, update_schema）、根拠、ターゲット、期待される成果、工数、検証シグナルを持つ構造化アクションアイテム
- **スレッド間関係** — overlaps_with, depends_on, contradicts, evolves_to, complements, competes_with, shares_gap, inspires — エビデンス参照と信頼度付き

**進化トリガー:**
- **新規ソースインジェスト** — 新しいコンテンツに関連するスレッドを自動更新
- **ユーザーコンテキスト追加** — ユーザーが探索の方向性や質問を提供
- **手動リフレッシュ** — 全再生成（3–12 スレッド）または単一スレッドの関連度フィルタリングイテレーション
- **定期進化** — 定期的なバックグラウンドスレッドメンテナンス

**スレッドゴミ箱:**
- 削除されたスレッドは全アーティファクト（ノード、エッジ、ギャップ、メインラインステップ、次の方向性、関係）と共に**ゴミ箱**へ移動し、復元用にタイムスタンプが記録される
- **復元**でスレッドとすべての関連データを完全に復旧
- **完全削除**でゴミ箱から恒久的に削除

### 19. クリエイティブ・インスピレーション・スイート

元の設計にはありません。Wiki 知識から創造的なアイデアを生成・進化させるマルチタブのインスピレーションワークスペースです。

**アイデアファクトリー:**
- SCAMPER、TRIZ、デザイン思考、グラフ構造的空隙、類推転移、反実仮想推論、エビデンス駆動、テーマ進化、ドリームウォークの 9 つの創造手法
- アイデア進化ステージ: seed → candidate → incubating → validated → mature
- **アイデアランキング**: 新規性、実現可能性、インパクト、エビデンスサポートでスコアリング
- **アイデア重複排除**: 類似概念をマージ
- 生成されたアイデアへのユーザーフィードバックとコメント

**テーマラボ:**
- コミュニティ検出とトピックマイニングによる Wiki コンテンツからのテーマ抽出
- LLM ガイドによるテーマ進化探索
- テーマクラスタ内でのシードベースのアイデア生成

**ドリームラボ:**
- 反実仮想と自由連想による創造的発想
- マルチステップのドリーム進化: 断片 → ウォーク → 統合
- 予想外のコンセプトペアリングによるクロスドメインインスピレーション

**成果:**
- 過去に生成されたアイデアのレビューと評価
- アイデアライフサイクルの追跡と過去のインスピレーションセッションの再訪問

### 20. WeChat FileHelper 統合

元の設計にはありません。WeChat FileHelper との完全な統合により、会話やメディアを直接 Wiki にインポートします。

- **QR コードログイン** — スキャンして WeChat FileHelper に認証
- **メッセージ履歴同期** — 過去の会話を閲覧・インポート
- **添付ファイルダウンロード** — WeChat 会話から画像、文書、ファイルを取得
- **カード/リンクプレビュー解析** — 共有リンクや記事を抽出・処理
- **アプリ内チャット** — アプリ内でメッセージを送受信
- **自動インジェスト** — インポートされたメッセージと添付ファイルは自動的にインジェストパイプラインで処理

### 21. 論文リサーチプラットフォーム

元の設計にはありません。専用の学術論文リサーチモジュールです。

- **論文 PDF インポート** — 直接の PDF アップロードと自動分析
- **文献検索** — LLM クエリ書き換えによる学術ソース向け Web 検索
- **arXiv デイリーモニタリング** — 研究関心に一致する新着論文の定期スキャン
- **論文候補収集** — インポート前の論文のバッチ収集とレビュー
- **論文分析** — LLM 生成のサマリー、主要発見、方法論抽出、既存 Wiki 知識との相互参照
- **PDF ダウンロード** — arXiv やその他オープンアクセスソースから論文を直接ダウンロード
...
| グラフ | sigma.js + graphology + ForceAtlas2 + Louvain |
| 検索 | トークン化検索 + グラフ関連度 + 任意のベクトル検索（LanceDB） |
| ベクトル DB | LanceDB（Rust、組み込み、オプション） |
| PDF | pdf-extract + pdfium-render（Rust） |
| Office | docx-rs + calamine |
| 多言語対応 | react-i18next（英語 + 中国語） |
| 状態管理 | Zustand 5 |
| LLM | ストリーミング fetch（OpenAI、Anthropic、Google、Azure、Ollama、DeepSeek、MiniMax、Claude Code CLI、Codex CLI、カスタム） |
| Web 検索 | Tavily、SerpApi、SearXNG、MiniMax、Ollama |
| WeChat | WeChat FileHelper プロトコル（QR ログイン、メッセージ同期、添付ファイルダウンロード） |
