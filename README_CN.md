...
- **知识脉络** — LLM 驱动的结构化知识演进系统，包含节点、边、缺口、主线步骤、下一步方向、脉络间关系和证据引用；支持按脉络独立迭代、关联度过滤和回收站机制
- **创意灵感套件** — 创意工坊（SCAMPER、TRIZ、设计思维、结构洞、类比迁移、反事实推理、证据驱动等 9 种方法论）、主题实验室和梦境实验室，支持多轮创意演化和反馈
- **微信文件助手集成** — 扫码登录、消息历史同步、附件下载、卡片/链接预览解析、应用内聊天
- **论文研究平台** — 学术论文 PDF 导入与自动分析、文献检索与 LLM 查询改写、arXiv 每日监测、论文候选收集与批量导入
...
原始设计中没有。基于 LLM 驱动的系统，将 Wiki 知识组织为持续演进的、结构化的**知识脉络**：

**脉络模型：**
- 每条脉络拥有**名称**、**摘要**（附证据引用）、**核心问题**（机制/权衡/演化/矛盾/应用/缺口六种类型）、**根主题**、**关键概念**和**源页面**
- **评分维度**追踪成熟度、覆盖度、一致性、新颖度和活跃度——每次迭代由 LLM 更新
- **状态生命周期**：形成中 → 活跃 → 成熟 → 陈旧 → 归档
- **生成模式**：`llm`（完整 LLM 分析）、`local_candidate`（启发式预填充）或 `repair`（修复）

**每条脉络的知识图谱：**
- **节点** 8 种类型：主题、概念、问题、方法、主张、案例、缺口、创意——每个节点附来源页面引用、置信度和重要性评分
- **边** 9 种关系类型：包含、依赖、支持、矛盾、演化至、启发、派生自、应探索、相关——每条边附推理说明和置信度
- **知识缺口** 结构化元数据：缺口类型、为何重要、缺失证据、来源节点/页面引用、优先级和状态（开放→调查中→已解决→已关闭→观察中）
- **主线步骤** —— 有序的故事线，从背景到核心概念、关键问题、方法、证据、案例、矛盾、缺口，直至下一步方向
- **下一步方向** —— 结构化行动项，包含动作类型（深入阅读/询问用户/网络搜索/连接节点/验证主张/对比脉络/生成创意/更新模式）、理由、目标、预期产出、工作量评估和验证信号
- **脉络间关系** —— 重叠/依赖/矛盾/演化至/互补/竞争/共享缺口/启发——附证据引用和置信度

**演化触发方式：**
- **新资料摄入** —— 自动更新与新内容相关的脉络
- **用户补充描述** —— 用户提供方向或问题供脉络探索
- **手动刷新** —— 全量重新生成（3–12 条）或单脉络关联度过滤迭代
- **定时演化** —— 周期性后台脉络维护

**脉络回收站：**
- 删除的脉络移入回收站，保留所有产物（节点、边、缺口、主线步骤、下一步方向、关系）并记录删除时间
- **恢复**操作完整还原脉络及其所有关联数据
- **永久删除**从回收站彻底清除

### 19. 创意灵感套件

原始设计中没有。多标签页灵感工作区，从 Wiki 知识中生成和演化创意：

**创意工坊：**
- 9 种创意方法：SCAMPER、TRIZ、设计思维、图谱结构洞、类比迁移、反事实推理、证据驱动、主题演化和梦境漫步
- 创意演化阶段：种子 → 候选 → 孵化中 → 已验证 → 成熟
- **创意排序**：按新颖度、可行性、影响力和证据支持度评分
- **创意去重**：合并相似概念
- 用户对生成创意的反馈和评论

**主题实验室：**
- 通过社区检测和主题挖掘从 Wiki 内容中提取主题
- LLM 引导的主题演化探索
- 主题聚类内的种子式创意生成
...
原始设计中没有。完整的微信文件助手集成，将对话和媒体直接导入 Wiki：

- **扫码登录** —— 扫码认证微信文件助手
- **消息历史同步** —— 浏览和导入历史对话
- **附件下载** —— 从微信对话中获取图片、文档和文件
- **卡片/链接预览解析** —— 提取和处理分享的链接和文章
- **应用内聊天** —— 在应用内收发消息
- **自动摄入** —— 导入的消息和附件自动通过摄入管线处理

### 21. 论文研究平台

原始设计中没有。独立的学术论文研究模块：

- **论文 PDF 导入** —— 直接上传 PDF 并自动分析
- **文献检索** —— 网络搜索配合 LLM 查询改写，精准获取学术来源
- **arXiv 每日监测** —— 周期性扫描匹配研究方向的新论文
- **论文候选收集** —— 批量收集和审核待导入论文
- **论文分析** —— LLM 生成摘要、关键发现、方法提取，与现有 Wiki 知识交叉引用
- **PDF 下载** —— 从 arXiv 等开放获取源直接下载论文
...
| 状态管理 | Zustand 5 |
| LLM | 流式 fetch（OpenAI、Anthropic、Google、Azure、Ollama、DeepSeek、MiniMax、Claude Code CLI、Codex CLI、自定义） |
| 网络搜索 | Tavily、SerpApi、SearXNG、MiniMax、Ollama |
| 微信 | 微信文件助手协议（扫码登录、消息同步、附件下载） |
